...
const Page = () => {  
 ...
 const hanndleToggleSwitch = async () => {
    try {
      const response = await fetch("api/accept-message",{
        ...
        body : JSON.stringify({AcceptingMessage : !acceptingMessege , userIds : session?.user.name}) 
      }) ; 
      const data = await response.json() ; 
      if(data.success) {
        toast({ 
 ...
 const showUrl = `${baseUrl}/u/${session?.user.username || session?.user.name}`  

 const handleDeleteMessage = async (messageId:unknown) => {
      setMessage(message.filter((mess) => mess._id !== messageId)) ; 
